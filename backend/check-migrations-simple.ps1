# ============================================================================
# Script de verificación pre-migración (Versión simplificada)
...
$null = & psql -h $env:DB_HOST -U $env:DB_USER -d $env:DB_NAME -p $env:DB_PORT -c "\q" 2>&1
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "Nota: Si PostgreSQL no está instalado localmente, esto es normal." -ForegroundColor Yellow
    Write-Host "      Las migraciones se ejecutarán cuando conectes al servidor.`n" -ForegroundColor Yellow
}
...
& npx knex migrate:status --env $Environment 2>&1
Write-Host ""
...
if (Test-Path $backupDir) {
    $backups = Get-ChildItem $backupDir -Filter "*.sql*" -ErrorAction SilentlyContinue | 
