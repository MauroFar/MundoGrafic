...
import { FaBell, FaCheckCircle, FaExclamationTriangle, FaInfo, FaTimesCircle, FaClock } from 'react-icons/fa';
...
const NotificacionesSistema = () => {
  ...
  useEffect(() => {
    ...
    const handleNuevaNotificacion = (event) => {
      ...
      // Auto-marcar como leída después de 5 segundos
      setTimeout(() => {
    ...
    const notificacionesGuardadas = localStorage.getItem('notificaciones-sistema');
  ...
  useEffect(() => {
    localStorage.setItem('notificaciones-sistema', JSON.stringify(notificaciones));
  }, [notificaciones]);
...
export default NotificacionesSistema;
