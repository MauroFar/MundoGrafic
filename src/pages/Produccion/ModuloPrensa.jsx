...
import { FaPrint, FaPlay, FaPause, FaStop, FaCheckCircle, FaExclamationTriangle, FaClock, FaCog } from 'react-icons/fa';

const ModuloPrensa = () => {
  const [ordenesPrensa, setOrdenesPrensa] = useState([]);
      ...
      const datosFicticios = [
        {
          ...
          cantidad: 1000,
          estado: 'en_proceso',
          maquina: 'Prensa Offset Heidelberg',
          operador: 'Carlos López',
          inicio: new Date(Date.now() - 2 * 60 * 60 * 1000), // Hace 2 horas
          estimado: new Date(Date.now() + 4 * 60 * 60 * 1000), // En 4 horas
          progreso: 35,
          prioridad: 'alta',
          observaciones: 'Papel especial requerido'
        },
        {
          id: 2,
          ...
          cantidad: 5000,
          estado: 'pendiente',
          maquina: 'Prensa Digital HP',
          operador: 'María García',
          inicio: null,
          estimado: new Date(Date.now() + 6 * 60 * 60 * 1000),
          progreso: 0,
          prioridad: 'media',
          observaciones: 'Tintas especiales para colores corporativos'
        },
        {
          id: 3,
          ...
          cantidad: 200,
          estado: 'completado',
          maquina: 'Prensa Offset Komori',
          operador: 'Roberto Silva',
          inicio: new Date(Date.now() - 8 * 60 * 60 * 1000),
          estimado: new Date(Date.now() - 2 * 60 * 60 * 1000),
          progreso: 100,
          prioridad: 'baja',
          observaciones: 'Papel laminado brillante'
        }
      ];
      
      setOrdenesPrensa(datosFicticios);
      setCargando(false);
    }, 1000);
  };

  const cambiarEstadoPrensa = (id, nuevoEstado) => {
    setOrdenesPrensa(prev => 
      prev.map(orden => 
        orden.id === id 
          ? { 
              ...orden, 
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-blue-50 p-4 rounded-lg">
          <div className="text-2xl font-bold text-blue-600">
            {ordenesPrensa.filter(o => o.estado === 'en_proceso').length}
          </div>
          <div className="text-sm text-blue-800">En Proceso</div>
        </div>
        <div className="bg-yellow-50 p-4 rounded-lg">
          <div className="text-2xl font-bold text-yellow-600">
            {ordenesPrensa.filter(o => o.estado === 'pendiente').length}
          </div>
          <div className="text-sm text-yellow-800">Pendientes</div>
        </div>
        <div className="bg-green-50 p-4 rounded-lg">
          <div className="text-2xl font-bold text-green-600">
            {ordenesPrensa.filter(o => o.estado === 'completado').length}
          </div>
          <div className="text-sm text-green-800">Completadas</div>
        </div>
        <div className="bg-purple-50 p-4 rounded-lg">
          <div className="text-2xl font-bold text-purple-600">
            {ordenesPrensa.reduce((sum, o) => sum + o.cantidad, 0)}
          </div>
          <div className="text-sm text-purple-800">Total Impreso</div>
        </div>
      </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex gap-2">
                      {orden.estado === 'pendiente' && (
                        <button
                          onClick={() => cambiarEstadoPrensa(orden.id, 'en_proceso')}
                      ...
                      {orden.estado === 'en_proceso' && (
                        <>
                          <button
                            onClick={() => cambiarEstadoPrensa(orden.id, 'pausado')}
                          ...
                          <button
                            onClick={() => cambiarEstadoPrensa(orden.id, 'completado')}
                            className="text-green-600 hover:text-green-900"
                            title="Completar"
                          >
                            <FaCheckCircle />
      ...
      {/* Observaciones */}
      <div className="mt-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Observaciones de Producción</h3>
        <div className="space-y-3">
          {ordenesPrensa.filter(o => o.observaciones).map((orden) => (
